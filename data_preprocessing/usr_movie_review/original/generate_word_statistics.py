...
class WordStatisticsGenerator:
    ...
    def get_documents(self):
        with open(os.path.join(self.data_path, "test.jsonl"), "r") as f:
    ...
    def save_scored_vocab(self, scored_vocab):
        with open(os.path.join(self.save_path, "scored_vocab__.pkl"), "wb") as f:
    ...
    def save_replacement_probs(self, replacement_probs):
        with open(os.path.join(self.save_path, "test_replacement_probs.pkl"), "wb") as f:
            pickle.dump(replacement_probs, f)

    def get_replacement_probs(self, documents):  # FIXED: Removed the extra 't'
        return [self.get_document_replacement_probs(document) for document in tqdm(documents)]

    def generate_replacement_probs(self):
        replacement_probs = self.get_replacement_probs(self.documents)  # FIXED: Call the correct method
